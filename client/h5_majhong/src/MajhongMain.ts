...
class MajhongMain extends egret.DisplayObjectContainer {

    public initLoadGroup:string = "majhong";
     ...
     * Create a game scene
     */
    public baseView:MajhongBattleView;
    ...
    {
        let majhong = this.baseView.data.testMajhong();
        while(this.baseView.data.handMajhongList.indexOf(majhong)!=-1)
        {
            majhong = this.baseView.data.testMajhong();
        }
        let card:MajhongCard = new MajhongCard(this.baseView.play_3.getTangType(),majhong,0);
        card.x = e.stageX;
        card.y = e.stageY;
